...
public class FileUtil {
	  ...
	  if(file.isDirectory()) {
			...
			for(File sonFile:sonFiles) {
				if(sonFile.isFile()) {
					...
					packages.add(path.substring(path.indexOf("classes")+8,path.lastIndexOf(".")).replaceAll("\\\\", "."));
 ...
 public static File getFile(String path) throws URISyntaxException {
	 return new File(FileUtil.classLoader.getResource(path).toURI());
 }
 
 public static Set<String> uploadFiles(HttpServletRequest request) throws IllegalStateException, IOException, ServletException, URISyntaxException {
